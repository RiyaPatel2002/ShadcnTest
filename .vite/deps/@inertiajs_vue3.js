...
function mergeWithDeep(target, source, merge4, stack) {
  ...
  for (let i = 0; i < sourceKeys.length; i++) {
    ...
    const merged = merge4(targetValue, sourceValue, key2, target, source, stack);
    if (merged !== void 0) {
      target[key2] = merged;
    } else if (Array.isArray(sourceValue)) {
      target[key2] = mergeWithDeep(targetValue, sourceValue, merge4, stack);
    } else if (isObjectLike(targetValue) && isObjectLike(sourceValue)) {
      target[key2] = mergeWithDeep(targetValue, sourceValue, merge4, stack);
    } else if (targetValue == null && isPlainObject2(sourceValue)) {
      target[key2] = mergeWithDeep({}, sourceValue, merge4, stack);
...
function transform(object, iteratee$1 = identity, accumulator) {
  ...
  forEach2(object, (value, key2, object2) => iteratee$1(accumulator, value, key2, object2));
  return accumulator;
}
...
function bindAll(object, ...methodNames) {
  ...
  for (let i = 0; i < methods.length; i++) {
    ...
    const stringKey = toString3(key2);
...
function append2(form, key2, value) {
  if (Array.isArray(value)) {
    return Array.from(value.keys()).forEach((index) => append2(form, composeKey(key2, index.toString()), value[index]));
...
function useForm(rememberKeyOrData, maybeData) {
  ...
  let defaults3 = typeof data === "function" ? cloneDeep(data()) : cloneDeep(data);
  let cancelToken = null;
  let recentlySuccessfulTimeoutId = null;
  let transform2 = (data2) => data2;
  const form = reactive({
    ...restored ? restored.data : cloneDeep(defaults3),
    isDirty: false,
    ...
    defaults(fieldOrFields, maybeValue) {
      ...
      if (typeof fieldOrFields === "undefined") {
        defaults3 = cloneDeep(this.data());
        this.isDirty = false;
      } else {
        defaults3 = typeof fieldOrFields === "string" ? set(cloneDeep(defaults3), fieldOrFields, maybeValue) : Object.assign({}, cloneDeep(defaults3), fieldOrFields);
      }
      return this;
    },
    reset(...fields) {
      const resolvedData = typeof data === "function" ? cloneDeep(data()) : cloneDeep(defaults3);
      ...
      if (fields.length === 0) {
        ...
        fields.filter((key2) => has(clonedData, key2)).forEach((key2) => {
          set(defaults3, key2, get(clonedData, key2));
  ...
  watch(
    form,
    (newValue) => {
      form.isDirty = !isEqual(form.data(), defaults3);
      if (rememberKey) {
